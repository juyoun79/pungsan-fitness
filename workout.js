  ...
  function renderDayDetail(d) {
    ...
    // 걸음수 기록 수집
    const todayStepsKey = 'steps_' + userId + '_' + dateStr;
    const stepsData = JSON.parse(localStorage.getItem(todayStepsKey) || 'null');
    if (stepsData && stepsData.steps > 0) {
      dayRecords.push({ type:'steps', record: stepsData });
    }
    ...
    for (const item of dayRecords) {
      if (item.type === 'steps') {
        const steps = item.record.steps || 0;
        const goal = item.record.goal || 10000;
        const kcal = item.record.kcal || 0;
        ...
        return `<div style="background:var(--card);border:1px solid var(--border);border-radius:var(--radius);padding:14px 16px;margin-bottom:10px;">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
            <div style="width:36px;height:36px;border-radius:10px;background:#EAF3DE;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">🚶</div>
            <div style="flex:1;">
              <div style="display:flex;align-items:center;gap:6px;">
                <span style="font-size:11px;font-weight:700;color:white;background:#3B6D11;padding:2px 6px;border-radius:5px;">걸음수</span>
                <span style="font-size:14px;font-weight:700;color:var(--text);">${steps.toLocaleString()}보</span>
              </div>
              <div style="font-size:12px;color:var(--text-sub);margin-top:2px;">목표 ${goal.toLocaleString()}보 · ${kcal}kcal</div>
            </div>
          </div>
          <div style="background:#C0DD97;border-radius:4px;height:6px;">
            <div style="background:#3B6D11;border-radius:4px;height:6px;width:${pct}%;"></div>
          </div>
          <div style="text-align:right;margin-top:4px;font-size:11px;color:#3B6D11;">${pct}% 달성</div>
        </div>`;
      }
  ...
  function loadTodaySteps() {
    const userId = localStorage.getItem('current_user');
    const today = getToday();
    const key = 'steps_' + userId + '_' + today;
    const saved = JSON.parse(localStorage.getItem(key) || '{"steps":0,"goal":10000}');
    stepCount = saved.steps || 0;
    stepGoal = saved.goal || 10000;
    updateStepsUI();
  }
  ...
  function updateStepsUI() {
    ...
    const barEl = document.getElementById('steps-progress-bar');
    const msgEl = document.getElementById('steps-msg');
    const kcalEl = document.getElementById('steps-kcal');
    if (el) el.textContent = stepCount.toLocaleString();
    if (goalEl) goalEl.textContent = stepGoal.toLocaleString();
    const pct = Math.min(100, Math.round((stepCount / stepGoal) * 100));
    if (barEl) barEl.style.width = pct + '%';
    const kcal = Math.round(stepCount * 0.04);
    if (kcalEl) kcalEl.textContent = kcal;
    let msg = '걷기를 시작해보세요! 💪';
    if (stepCount >= stepGoal) msg = '목표 달성! 대단해요! 🎉';
    else if (pct >= 80) msg = '거의 다 왔어요! 조금만 더!';
    else if (pct >= 50) msg = '절반 넘었어요! 잘하고 있어요!';
    else if (pct >= 20) msg = '좋은 출발이에요!';
  ...
  // 홈 화면 걸음수 카드 업데이트
  function updateHomeStepsCard() {
    const userId = localStorage.getItem('current_user');
    const today = getToday();
    const key = 'steps_' + userId + '_' + today;
    const saved = JSON.parse(localStorage.getItem(key) || '{"steps":0,"goal":10000}');
    const steps = saved.steps || 0;
    const goal = saved.goal || 10000;
    const pct = Math.min(100, Math.round((steps / goal) * 100));
    const kcal = Math.round(steps * 0.04);
    const stepsEl = document.getElementById('stat-steps');
    const barEl = document.getElementById('stat-steps-bar');
    const msgEl = document.getElementById('stat-steps-msg');
    const kcalEl = document.getElementById('stat-steps-kcal');
    const goalEl = document.getElementById('stat-steps-goal');
    if (stepsEl) stepsEl.textContent = steps.toLocaleString();
    if (goalEl) goalEl.textContent = goal.toLocaleString();
    if (barEl) barEl.style.width = pct + '%';
    if (kcalEl) kcalEl.textContent = kcal;
    let msg = '걷기를 시작해보세요!';
    if (steps >= goal) msg = '목표 달성! 🎉';
    else if (pct >= 80) msg = '거의 다 왔어요!';
    else if (pct >= 50) msg = '절반 넘었어요!';
    else if (pct >= 20) msg = '좋은 출발이에요!';
    if (msgEl) msgEl.textContent = msg;
  }
  ...
  function saveSteps() {
    ...
    const today = getToday();
    const key = 'steps_' + userId + '_' + today;
    const data = { steps: stepCount, goal: stepGoal, kcal: Math.round(stepCount * 0.04), date: today };
    localStorage.setItem(key, JSON.stringify(data));
    // Firebase 저장
    if (typeof db !== 'undefined') {
      db.ref('users/' + userId + '/steps/' + today).set(data);
    }
    updateHomeStepsCard();
  }

  // 걸음수 목표 변경
  function changeStepGoal() {
    const input = document.getElementById('steps-goal-input');
    const val = parseInt(input?.value) || 10000;
    if (val < 1000 || val > 100000) { alert('1,000 ~ 100,000보 사이로 입력해주세요!'); return; }
    stepGoal = val;
    saveSteps();
    updateStepsUI();
    alert('목표가 ' + val.toLocaleString() + '보로 설정됐어요! 💪');
  }
  ...
  function startStepTracking() {
    ...
    if (typeof DeviceMotionEvent !== 'undefined' && typeof DeviceMotionEvent.requestPermission === 'function') {
      DeviceMotionEvent.requestPermission().then(state => {
        if (state === 'granted') { _startMotionListener(); }
        else { alert('동작 감지 권한이 필요해요. 설정에서 허용해주세요.'); }
      }).catch(() => { _startMotionListener(); });
    } else {
  ...
  function _startMotionListener() {
    stepTracking = true;
    stepLastMag = 0;
    const btn = document.getElementById('steps-track-btn');
    if (btn) { btn.textContent = '⏹ 측정 중지'; btn.style.background = '#ef4444'; }
    stepListener = function(e) {
      const acc = e.accelerationIncludingGravity;
      if (!acc) return;
      const mag = Math.sqrt(acc.x*acc.x + acc.y*acc.y + acc.z*acc.z);
      const diff = Math.abs(mag - stepLastMag);
      if (diff > 2.5 && stepLastMag > 0) {
        stepCount++;
        updateStepsUI();
        if (stepCount % 10 === 0) saveSteps();
      }
      stepLastMag = mag;
    };
    window.addEventListener('devicemotion', stepListener);
    const statusEl = document.getElementById('steps-status');
    if (statusEl) statusEl.textContent = '측정 중... 핸드폰을 들고 걸어주세요!';
  }

  // 만보계 중지
  function stopStepTracking() {
    stepTracking = false;
    if (stepListener) { window.removeEventListener('devicemotion', stepListener); stepListener = null; }
    const btn = document.getElementById('steps-track-btn');
    if (btn) { btn.textContent = '▶ 걸음수 측정 시작'; btn.style.background = '#3B6D11'; }
    const statusEl = document.getElementById('steps-status');
    if (statusEl) statusEl.textContent = '측정이 중지됐어요.';
    saveSteps();
  }

  // 측정 시작/중지 토글
  function toggleStepTracking() {
    if (stepTracking) stopStepTracking();
    else startStepTracking();
  }

  // 걸음수 초기화
  function resetSteps() {
    if (!confirm('오늘 걸음수를 초기화할까요?')) return;
    stepCount = 0;
    saveSteps();
    updateStepsUI();
  }
